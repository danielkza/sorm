  ...
  with StdNow
{
  ...
  override def insertAndGetGeneratedKeys(table: String, values: Iterable[(String, Any)], keys: Iterable[String])
    = super.insertAndGetGeneratedKeys(table, values, keys).take(1)
}